...
$.extend($.jgrid,{
	...
	msie : navigator.appName === 'Microsoft Internet Explorer' || !!window.ActiveXObject || "ActiveXObject" in window,
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			scrollrows : false,
			autowidth: false,
			scrollOffset :$.jgrid.msie ? 20 : ($.jgrid.firefox ? 20 : 8),
			cellLayout: 5,
			subGridWidth: 20,
			multiselectWidth: 20,
			gridview: false,
...
//			if(rdata[nm] && rdata[nm].substr(0,1) == '<'){
//				result += $(rdata[nm]).attr('style')+';';
//			}
			if(rowInd===0) {
...
//			}
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			while (i<len) {
				...
				altr = rcnt === 1 ? 0 : rcnt;
				cn1 = (altr+i)%2 === 1 ? cn : 'ui-priority-firstary';
				if( selected) {
					if( ts.p.multiselect) {
			...
			{
				//(#19482 by zuoyong.tang)
	            if(!ts.id || ts.id.indexOf('dl_list_grid') < 0 )
	            {
	            	if(!ts.p.fixedScroll || ts.p.fixedScroll == false){
		...
		setPager = function (pgid, tp){
			...
			clearVals = function(onpaging){
				...
				ts.p.savedRow = [];
				//if(ret==='stop') {return false;} //initial position
				return true;
			};
			pgid = pgid.substr(1);
			tp += "_" + pgid;
			...
			$('input.ui-pg-input',"#"+pgcnt).keypress( function(e) {
				...
				if(key === 13) {
					...
					if (parseInt($(this).val())>ts.p.lastpage){
		...
		sortData = function (index, idxcol,reload,sor, obj){
			...
			if(!reload) {
				...
				} else { ts.p.sortorder = ts.p.colModel[idxcol].firstsortorder || $("span.s-ico>span:not(.ui-state-disabled)",obj).length>0?$("span.s-ico>span:not(.ui-state-disabled)",obj).eq(0).attr("sort")==='asc'?'desc':'asc':'asc'; }//envision multisort hack
				ts.p.page = 1;
			}
			if(ts.p.multiSort) {
				...
				$("span.ui-grid-ico-sort",newSelectedTh).addClass('ui-state-disabled');//envision multisort hack
			...
			$('#cb_'+$.jgrid.jqID(ts.p.id),this).bind('click',function(){
				...
				if (this.checked) {
					$(ts.rows).each(function(i) {
						if (i>0) {
							if(!$(this).hasClass("ui-subgrid") && !$(this).hasClass("jqgroup") && !$(this).hasClass('ui-state-disabled') && this.id){
					...
					$(ts.rows).each(function(i) {
						if(i>0) {
							if(!$(this).hasClass("ui-subgrid") && !$(this).hasClass('ui-state-disabled') && this.id){
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					for(i = gi+si+ni; i < t.p.colModel.length;i++){
						cm = t.p.colModel[i];
						nm = cm.name;
//						if(data[nm].substr(0,1) == '<'){
//							data[nm] = $(data[nm]).text();
//						}
						lcdata[nm] = data[nm];
	...
	setGridHeight : function (nh) {
		return this.each(function (){
			...
			if($t.p.frozenColumns === true){
				...
				//判断浏览器，浏览器不同，浏览器滚动条的height也不同  by tao.jiang
				var a = navigator.userAgent.toLowerCase();
				var isIE = !(a.indexOf('opera') > -1) && a.indexOf('msie') > -1;
				$('#'+$.jgrid.jqID($t.p.id)+"_frozen").parent().height(bDiv.height() - (isIE?16:10));
			}
			$t.p.height = nh;
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			*/
			...
			// (#25543 20150506 by zuoyong.tang) 树形数据列表支持列固定
			//分组表格支持列冻结
			...
			if( maxfrozen>=0 && frozen) {
				...
				//(#20710 by zuoyong.tang)
				//表格处于影藏态的时候，hth计算出错
		        if($t.p.id && $t.p.id.indexOf('dl_list_grid') >= 0 && !hth)
		        {
		        	hth = 38;
		        }
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		var ahr= $("<a style='cursor:pointer' class='ui-jqdialog-titlebar-close ui-corner-all'></a>")
			...
			case "button" :
				...
				if(eltype !== "button"){
					if(autowidth) {
						...
						else {$(elem).attr("size",20);options.size = 20;}
		...
		return this.each(function(){
			...
			function createData(rowid,obj,tb,maxcols){
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && ((typeof this.editable === 'function' && this.editable.call($(obj),rowid)===true) || this.editable===true) && nm !== 'rn') {
						if(ind === false) {
							tmp = "";
						} else {
			...
			function fillData(rowid,obj,fmid){
				...
				$('td[role="gridcell"]',tre).each( function(i) {
					nm = cm[i].name;
					// hidden fields are included in the form
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && ((typeof cm[i].editable === 'function' && cm[i].editable.call($(obj),rowid)===true) || cm[i].editable===true)) {
						if(nm === obj.p.ExpandColumn && obj.p.treeGrid === true) {
							tmp = $(this).text();
						} else {
							try {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined) {
				...
				$("#dData","#"+dtbl+"_2").click(function(){
					...
					if(ret[0] && !rp_ge[$t.p.id].processing) {
						...
						postd=$.extend( postd, $t.p.postData);
						...
						if (!$(this).hasClass('ui-state-disabled')) {
							...
							if(dr){
								if($.isFunction( o.delfunc )){
									...
									pDel = $.extend(true,pDel,
										{
											top : $t.p.height*1/3,
											left: ($t.p.width-240)/2,
										});
...
$.jgrid.extend({
//Editing
	editRow : function(rowid,keys,oneditfunc,successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc, isFocus) {
			...
			if (editable === "0" && !$(ind).hasClass("not-editable-row")) {
				...
				$('td[role="gridcell"]',ind).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
						if($t.p.autoencode) { tmp = $.jgrid.htmlDecode(tmp); }
						svr[nm]=tmp;
						if((typeof cm[i].editable === 'function' && cm[i].editable.call($($t),rowid)===true) || cm[i].editable===true) {
		...
		if (editable==="1") {
			...
			if (o.url === 'clientArray') {
				...
				$.ajax($.extend({
					...
					type: (o.url && o.url != null) ? o.mtype : "GET", //bowen.yin  #25084 2015-02-06
					async : false, //?!?
	...
	restoreRow : function(rowid, afterrestorefunc) {
		...
		return this.each(function(){
			...
			if(fr >= 0) {
				...
				$.each($t.p.colModel, function(){
					if(((typeof this.editable === 'function' && this.editable.call($($t),rowid)===true) || this.editable===true) && $t.p.savedRow[fr].hasOwnProperty(this.name)) {
...
//						var sr = $t.p.savedRow[0].id;
						...
						if(sr) {
							...
							if($("#"+$.jgrid.jqID(sr), "#"+gID ).hasClass("jqgrid-new-row")) {
								...
								o.addParams.addRowParams.extraparam=$.extend( o.addParams.addRowParams.extraparam, this.p.postData);
								...
								o.editParams.extraparam=$.extend( o.editParams.extraparam, this.p.postData);
...
//						if($t.p.selrow === o.addParams.rowID ) {
						if($("#"+$.jgrid.jqID($t.p.selrow), "#"+gID ).hasClass("jqgrid-new-row")) {
		...
		return this.each(function(){
			...
			while(i<len) {
				...
				if(curLevel !== parseInt($t.p.tree_root_level,10)) {
					...
					// (#25543 20150506 by zuoyong.tang)
					// 加载树的时候，爷爷节点是非展开，父节点是展开，则爷爷的子孙节点都不显示，实际是爷爷的孙子节点都显示了
					// 为解决此问题，加了display_node属性
					 if( !expan || (pn && 'none' == pn.display_node)){
						$($t.rows[i]).css("display","none");
						ldat.display_node = 'none';
					}
					else
					{
						ldat.display_node = undefined;
					}
				}
	...
	expandRow: function (record){
		this.each(function(){
			...
			$(childern).each(function(){
				...
				if($t.grid.fbDiv)
				{
					$t.grid.fbDiv.find('[id="' + id + '"]').css("display","");
				}
	...
	collapseRow : function (record) {
		this.each(function(){
			...
			$(childern).each(function(){
				...
				if($t.grid.fbDiv)
				{
					$t.grid.fbDiv.find('[id="' + id + '"]').css("display","none");
				}
		...
		return this.each(function(){
			...
			if(!rc[expanded]) {
				...
				if( $(this).jqGrid("isNodeLoaded",this.p.data[position]) ) {
					...
					// (#25543 20150506 by zuoyong.tang) 树形数据列表支持列固定
					var rc1_fixed = $("#" + this.p.idPrefix + $.jgrid.jqID(id), this.grid.fbDiv)[0];
					rc1 = $(rc1).add(rc1_fixed);
...
$.jgrid.extend({
	...
	groupingToggle : function(hid){
		this.each(function(){
			...
			minus = grp.minusicon,
			plus = grp.plusicon,
			tar = $("[id="+$.jgrid.jqID(hid)+"]"),
			r = tar.length ? tar[0].nextSibling : null,
			//分组列固定 By Andy 
			frozen_r = tar.length > 1 ? tar[1].nextSibling : null,
...
$.jgrid.extend({
    ...
    columnChooser : function(opts) {
        ...
        for (var j = 0; j < colModel.length; j++) {
                     if(colModel[j].index == "WTG.Name" || colModel[j].index == 'ToTop'){
                     	fixed_pos[j] = true;
                     }
                 }
        
        
        //(#17594 by zuoyong.tang)
        if(self[0].p.id && self[0].p.id.indexOf('dl_list_grid') >= 0)
        {
			fixedCols = [2];
        }
        ...
        $.each(colModel, function(i) {
            ...
            if(self[0].p.id && self[0].p.id.indexOf('dl_list_grid') >= 0 && fixed_pos[i])
            {
     			return;
            }
